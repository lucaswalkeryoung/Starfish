...
class Linker(Visitor):
    ...
    # -----------------------------------------------------------------------------------------
    # ----------------------------- LINKER :: Base Linker Method ------------------------------
    # -----------------------------------------------------------------------------------------
    def visit_node(self, node: Node, context: Context) -> Node:

        for child in node.components:

            child.edges['parent'].append(node)
            node.edges['child'].append(child)

            child.visit(self, context.descend())

        return node


    # -----------------------------------------------------------------------------------------
    # ------------------------------ ENTRY :: Link "Within" Edge ------------------------------
    # -----------------------------------------------------------------------------------------
    def visit_zone(self, node: Zone, context: Context) -> Zone:

        for child in node.components:

            child.edges['within'].append(node)
            node.edges['contains'].append(child)

        return self.visit_node(node, context)


    # -----------------------------------------------------------------------------------------
    # --------------------- ENTRY :: Bind Players to Turns, Phases, Steps ---------------------
    # -----------------------------------------------------------------------------------------
    def visit_clock(self, node: Clock, context: Context) -> Clock:

        players = context.get('players')
        turns   = node.components
        ...
        for step_a, step_b in zip(steps_a, steps_b):
            step_a.edges['next'].append(step_b)
            step_b.edges['prev'].append(step_a)

        node.edges['now'].append(steps[0])

        return self.visit_zone(node, context)


    # -----------------------------------------------------------------------------------------
    # ---------------------------- ENTRY :: Capture World Context -----------------------------
    # -----------------------------------------------------------------------------------------
    def visit_world(self, node: 'World', context: Context) -> 'World':

        context.set('world', node)
    ...
    # -----------------------------------------------------------------------------------------
    # -------------------- ENTRY :: Capture Player Context and Link Turns ---------------------
    # -----------------------------------------------------------------------------------------
    def visit_player(self, node: 'Player', context: Context) -> 'Player':

        context.set('player', node)
        return self.visit_node(node, context)
    ...
    # -----------------------------------------------------------------------------------------
    @staticmethod
    def link(root: 'World') -> 'World':
        return root.visit(Linker(), Context())