...
class Auditor(Visitor):
    ...
    # -----------------------------------------------------------------------------------------
    # --------------------------- VISITOR :: Visit a Play Mechanic ----------------------------
    # -----------------------------------------------------------------------------------------
    def visit_play(self, node: 'Play', context: 'Context') -> None:
        context.get('actions').append(node)


    # -----------------------------------------------------------------------------------------
    # -------------------------- VISITOR :: Initialize Card Context ---------------------------
    # -----------------------------------------------------------------------------------------
    def visit_card(self, node: 'Card', context: 'Context') -> None:

        context.set('card', node)
        self.visit_node(node, context)


    # -----------------------------------------------------------------------------------------
    # ----------------------------- ENTRY :: Initialize an Audit ------------------------------
    # -----------------------------------------------------------------------------------------
    @staticmethod
    def audit(world: 'World', player: 'Player') -> defaultdict[str, list['Node']]:

        context = Context()
        context.set('player',  player)
        context.set('actions', dict())
        context.set('world',   world)

        return world.visit(Auditor(), context)