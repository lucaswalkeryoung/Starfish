...
class Printer(Visitor):
    ...
    # -----------------------------------------------------------------------------------------
    # --------------------------- VISITOR :: Base Visitation Method ---------------------------
    # -----------------------------------------------------------------------------------------
    def visit_node(self, node: 'Node', context: 'Context') -> str:
        ...
        output.append(node.__class__.__name__)
        output.append('(')
        ...
        if node.components:
            ...
            for component in node.components:
                ...
                output.append('\n')
                output.append(component.visit(self, context.descend()))
        ...
        if node.attributes:
            ...
            attributes = str(node.attributes)
    ...
    def visit_card(self, node: 'Node', context: 'Context') -> str:
        return self.visit_node(node, context)[:-1]

    @staticmethod
    def print(node: 'Node', full: bool = False) -> str:

        context = Context()
        context.set('entry', node)
        context.set('full',  full)

        return node.visit(Printer(), context)